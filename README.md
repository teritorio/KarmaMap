...
└── user_reputation_distribution.parquet # per-aspect CDF samples, see below
```
...
- `user_reputation_distribution.parquet` — per-aspect equal-mass CDF samples:

  | Column | Type | Meaning |
  |---|---|---|
  | `aspect` | `string` | `node_created`, `way_created`, `relation_created`, or `tag_<key>` for each Top12 tag |
  | `value` | `uint64` | A raw per-uid total on that aspect |
  | `less` | `uint64` | Contributors with a total strictly below `value` |
  | `active` | `uint64` | Contributors with a nonzero total on that aspect |

  Rows are sorted by the aspect declaration order (created counters, then
  tags) and by `value` within an aspect. The users viewer rank-scored a
  contributor against the full table before; this file is the same percentile
  delivered compactly: each aspect's population is sampled with up to 1024
  equal-mass windows, so a lookup underestimates the true rank by at most one
  window (≤ 0.1 percentage points), and populations ≤ 1024 are exact.
...
├── user_reputation_distribution.parquet  # only with --user-indicators
└── changes/
    └── year=2025/
        ├── month=01/
...
```json
{
  ...
  "datasets": {
    ...
    "user_reputation_distribution": { "path": "user_reputation_distribution.parquet", "partitions": [] }
  }
}
```
...
- **`/users/`** — the users viewer: look up an OSM username to see their
  ...
  `--user-indicators`. Ranking reads only `user_reputation_distribution.parquet`
  (≤ 15 × 1024 rows), not the full indicator table, so a planet-scale index
  loads in the browser without a full download.
