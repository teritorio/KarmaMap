...
- `user_reputation.parquet` — one row per user, sorted by `uid`:
  ...
  | Column | Type | Meaning |
  |---|---|---|
  | `<aspect>_points` | `float64` | Points earned: `cap · P` |
  | `<aspect>_pct` | `float64` | Percentile rank `100 · P` |
  | `<aspect>_active` | `uint64` | Contributors with a nonzero total on that aspect |
  | `<aspect>_max` | `uint64` | Largest per-user total on that aspect |

  (84 columns total.) Every aspect is computed exactly in C++: `P` is the
  ...
  sampled and the percentages match the full-table rank exactly. The viewer
  reads only the searched uid's rows (uid-range filter), so a planet-scale
  index loads in the browser without downloading the indicator table.
...
- **`/users/`** — the users viewer: look up an OSM username to see their
  ...
  `--user-indicators`. The reputation is read exactly from a single
  `user_reputation.parquet` file (one uid-sorted row per user, fetched with a
  uid-range filter), so a planet-scale index loads in the browser without a
  full download of the indicator table.
